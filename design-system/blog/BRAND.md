...
> 来源：实例化自 Li-Design V1.4 可复用品牌模板（git 子模块 `design-system/Li-Design`）；令牌值已与 `reusable-tokens.template.css` 逐项核对，另含打印/代码高亮/CTA 等本站扩展
...
| 11 | 语义色 | Li-Design V1.4 调校值：success #2A7C52 / warning #9A5C05 / destructive #C43737（深色 #86D6AC / #EAD48E / #E8A49A，各配 soft；深色带文字组件回退 soft-solid / soft-fg） |
...
| 模板参考 | design-system/Li-Design/（git 子模块，V1.4） | 家族模板/组件规格/验收清单；仅参考，非运行时依赖 |
...
- 本方案已实例化，Li&Blog 不再依赖 Li-Design 模板仓库作为运行时依赖；`design-system/Li-Design/` 仅作模板参考（git 子模块，升级需显式更新锁定提交）
