...
- 2026-08-24 移动端导航收敛为二级菜单（首页/关于/搜索 + 友情链接），页脚增加友情链接入口；友情链接仍属导航，不引入任何用户输入组件；外链统一经过站内安全提醒页确认后再打开，公开站继续保持零交互栏目。
- 2026-08-24 公开站视觉重构：更大留白与一致栅格、液态玻璃卡片（`--liblog-glass-*`）、首页/项目页 CSS 瀑布流与确定性抽象视觉块、卡片悬浮微交互、搜索微骨架屏、全站带插图的空状态；仍遵守公开站零输入、`prefers-reduced-motion` 收敛、颜色只进 tokens.css 的底线。
