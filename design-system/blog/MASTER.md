...
- 令牌校验（2026-08-21，1:1）：模板 57 个 `--{{PROJECT_PREFIX}}-*` 变量集合与本站 `--liblog-*` 全量对比，**0 缺失**；57 个共有变量逐值一致；V1.5 页脚规格以 `--liblog-footer-bg` / `--liblog-footer-border` / `--liblog-footer-blur` 三枚令牌落地（模板 Tailwind 类 → 本站令牌等价，明暗两套）；`--liblog-print-*` / `--liblog-code-*` / `--liblog-btn-light/dark-*` / `--liblog-badge-readme-fg` / `--liblog-glass-*`（2026-08-24 液态玻璃）为本站扩展
...
| masonry | 首页/项目页瀑布流（CSS `columns`，三列 → 两列 → 单列；卡片视觉块宽高比错落，浏览器原生布局零 JS、零依赖） |
| search-skeleton | 搜索索引异步加载微骨架：三行灰色占位条 + shimmer，加载完成/失败后自动切换为结果或错误文案（`strings.common.search_loading / search_error`） |
| empty-state | 全站空状态组件：内联 SVG 小图 + 引导文案 + 返回首页动作，覆盖文章/项目/时间线/标签/归档/友链（文案走 `strings.common.empty_*`） |
| archive-page | 归档页 `/archive/`：按年分组与计数，纯派生内容，页脚入口（文案走 strings.nav.archive） |
| friends-page | 友情链接 `/friends/`（2026-08-24）：由 `content/friends/*.md` 派生，`weight` 升序卡片列表，空状态/入口文案走 `strings.yaml`；每站详情页即外链安全提醒页 `/friends/<slug>/`，展示目标域名与提示，用户确认后才以 `rel="noopener noreferrer nofollow"` 打开站外地址 |
...
**P2 已实现（2026-08-18）：** Setup 三步向导、本地登录（限速+CSRF）、Li&Pass OIDC 登录/绑定/回程登出、九栏目（文章/项目/时间线/关于/资源/友情链接/品牌/文案/首页/资料）、Hugo 预览与保存重建；16 项单元测试通过 + 端到端 curl 流程验证。
...
| 全站空状态插图 + 引导文案 | strings.yaml（`common.empty_*` / `common.browse_home`）+ `partials/empty-state.html` 内联 SVG | 页面文案 |
| 搜索骨架加载/失败文案 | strings.yaml（`common.search_loading` / `common.search_error`） | 页面文案 |
...
| 友情链接条目（名称/地址/简介/排序） | friends Markdown frontmatter（`title`/`href`/`description`/`weight`） | 友情链接（新建/编辑/查看/删除/回收站） |
| 友情链接页标题/简介/空状态 | strings.yaml（`friends.*`） | 页面文案 |
| 外链安全提醒文案与按钮 | strings.yaml（`friends.safety_*`） | 页面文案 |
| 移动菜单入口与页脚友情链接按钮文案 | strings.yaml（`nav.menu` / `nav.friends`） | 页面文案 |
...
| 页脚友情链接入口 | strings.yaml（nav.friends） | 页面文案 |
...
**整卡可点击（2026-08-22）：** 文章卡/项目卡整卡可点击进入详情（标题链接 Stretched Link 拉伸覆盖整卡，纯 CSS `::after` 实现）；卡内标签链接与项目仓库按钮提升层级保持原有点击行为；键盘聚焦标题链接时整卡仍高亮。零新依赖、无 JS、技术栈不变；`make check` 通过，无头 Chromium 实测文章/项目卡摘要区、项目卡状态区点击均进入详情，标签与仓库按钮仍可点击。
...
**文章侧边目录（2026-08-22）：** 文章页目录悬浮在内容区外侧的视口左侧留白（`position: fixed`，`left: max(16px, calc(50vw - 628px))`），不占用/挤压正文位置（正文保持 720px 居中不变）；自动识别 Markdown 二级/三级标题（`hugo.toml` 显式 `[markup.tableOfContents] startLevel=2 endLevel=3`）；有标题即显示（不再要求 ≥300 词）；≤1280px 折叠为正文上方目录框，打印隐藏目录。纯 CSS、零 JS，动效尊重 `prefers-reduced-motion`；Hugo v0.165.0 构建 + 无头 Chromium 实测通过。
...
| web/src/tokens.css | themes/blog-theme/static/css/tokens.css（Tailwind CSS 4 编译，结构与 Panel index.css 同构；`npm run css`） |
