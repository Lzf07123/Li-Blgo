...
**文章侧边目录（2026-08-22）：** 文章页目录改为左侧固定侧边栏（`article-layout` 两栏栅格 + `position: sticky`，目录/正文显式同列同行，避免自动排布产生 h1 下空白），自动识别 Markdown 二级/三级标题（`hugo.toml` 显式 `[markup.tableOfContents] startLevel=2 endLevel=3`）；有标题即显示（不再要求 ≥300 词）；≤1024px 折叠为正文上方目录框，打印隐藏目录。纯 CSS、零 JS，动效尊重 `prefers-reduced-motion`；Hugo v0.165.0 构建 + 无头 Chromium 实测通过。
