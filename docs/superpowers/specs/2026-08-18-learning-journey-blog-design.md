...
7. 关于我：profile 表单 + 长文
8. 资源：条目 CRUD
9. 友情链接：站点名称/地址/简介/排序 CRUD（仅接受 http/https，保存即重建）
...
- 移动端二级菜单与友情链接（2026-08-24）：移动端首页/关于/搜索常驻页眉，文章/项目/历程/资源/友情链接收进二级菜单；桌面页眉资源后新增友情链接；友情链接以 `content/friends/*.md` 为唯一事实来源，公开站外链先经过站内安全提醒页确认再打开
...
已闭合决策：Hugo 分段构建（Go 生成器 + Python 编排壳）+ FastAPI 后台 + SQLite、Docker Compose profiles、双登录（本地 + OIDC）、单管理员、Setup 向导、九栏目后台（新增友情链接 CRUD）、首页聚合、兄弟项目纳入、全 Markdown、本地徽章、海玻璃家族令牌、氛围浓度分层、本地搜索、匿名统计、回程登出、分页 20 篇/页、索引瘦身、图片上传、外链安全提醒页、液态玻璃卡片、CSS 瀑布流、搜索微骨架、全站空状态、密码哈希 PBKDF2-HMAC-SHA256（600k，零原生依赖）。
