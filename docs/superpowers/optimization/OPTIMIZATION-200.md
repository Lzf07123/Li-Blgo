...
| v124 | 后台 | 文章回收站（软删除/恢复/清空） | 待办 | |
| v125 | 后台 | 复制为新草稿 | 待办 | |
| v126 | 后台 | 定时发布状态（scheduled）与提示 | 待办 | |
| v127 | 后台 | 编辑器 SEO 检查面板 | 待办 | |
| v128 | 后台 | 预览设备宽度切换（移动/平板/桌面） | 待办 | |
| v129 | 后台 | 编辑器快捷键帮助（?） | 待办 | |
| v130 | 后台 | slug 唯一性即时检查 | 待办 | |
| v131 | 后台 | 重复标题即时警告 | 待办 | |
| v132 | 后台 | 媒体库未引用筛选 | 待办 | |
| v133 | 后台 | 批量加标签/清除标签 | 待办 | |
| v134 | 后台 | 列表每页条数持久化 | 待办 | |
| v135 | 后台 | 统计路径映射文章标题 | 待办 | |
| v136 | 后台 | 备份页显示最近备份/恢复记录 | 待办 | |
| v137 | 后台 | 配置表单 YAML 前端语法校验 | 待办 | |
| v138 | 后台 | 后台全局快速跳转搜索框 | 待办 | |
| v139 | 后台 | 编辑字数统计区分中英文 | 待办 | |
| v140 | 后台 | 编辑器插入表格工具栏 | 待办 | |
| v141 | 后台 | 自动保存冲突检测（多标签页） | 待办 | |
| v142 | 后台 | 后台页面标题统一为「栏目 · Li&Blog 后台」 | 待办 | |
| v143 | 后台 | 后台导航移动端抽屉 | 待办 | |
| v144 | 内容 | 文章列表卡片补更新日期 | 待办 | |
| v145 | 内容 | 时间线列表按年份分组 | 待办 | |
| v146 | 内容 | 项目卡补日期与状态徽章 | 待办 | |
| v147 | 内容 | 关于页结构化技能列表 | 待办 | |
| v148 | 内容 | RSS 全文/摘要开关 | 待办 | |
| v149 | 内容 | 404 页补时间线入口 | 待办 | |
| v150 | 内容 | 搜索无结果时推荐标签 | 待办 | |
